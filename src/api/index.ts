  ...
  private setupRoutes(): void {
    ...
    this.app.post<{ Body: CreateTaskBody; Reply: TaskResponse | ErrorResponse }>(
      ...
      async (request, reply) => {
        ...
        const result = await this.mcpServer.handleToolCall("tensionai_execute", {
          prompt: body.prompt,
    ...
    this.app.get<{ Querystring: { status?: string; limit?: string }; Reply: TaskResponse[] }>(
      ...
      async (request) => {
        ...
        const tasks = await this.mcpServer.handleToolCall("tensionai_list_tasks", {
          status,
          limit,
        }) as TaskResult[];
    ...
    this.app.get<{ Params: { id: string }; Reply: TaskDetailResponse | ErrorResponse }>(
      ...
      async (request, reply) => {
        ...
        const task = await this.mcpServer.handleToolCall("tensionai_status", {
          taskId,
    ...
    this.app.delete<{ Params: { id: string }; Reply: { success: boolean; taskId: string } }>(
      ...
      async (request) => {
        ...
        const result = await this.mcpServer.handleToolCall("tensionai_abort", {
          taskId,
